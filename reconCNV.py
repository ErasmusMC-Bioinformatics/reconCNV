...
parser = argparse.ArgumentParser(
    description="Visualize CNV data from short read sequencing data."
)
...
parser.add_argument(
    ...
    dest="out_dir",
    default=None,
    help="Directory to place output files." " [Required]",
)
        ...
        for index, row in chr_boundary.iterrows():
            ...
            if vaf == True:
                text = Label(
                    ...
                    y=0.1,
                    text=str(
                        row[config["files"]["ratio_file"]["column_names"]["chromosome"]]
                    ),
                    text_color=config["plots"]["chromosome_boundaries"]["text_color"],
                    text_font_size=config["plots"]["chromosome_boundaries"][
                        "text_font_size"
                    ],
                    background_fill_alpha=config["plots"]["chromosome_boundaries"][
                        "text_background_alpha"
                    ],
                    background_fill_color=config["plots"]["chromosome_boundaries"][
                        "text_background_color"
                    ],
                    text_font_style=config["plots"]["chromosome_boundaries"][
                        "text_font_style"
                    ],
                )
            else:
...
data = pd.read_csv(
    options.ratio_file, sep=config["files"]["ratio_file"]["field_separator"]
)
logging.info("Successfully read the ratio file.")

# make sure chromosome names are string data type
data[config["files"]["ratio_file"]["column_names"]["chromosome"]] = data[
    config["files"]["ratio_file"]["column_names"]["chromosome"]
].astype(str)
...
if options.recenter:
    data[config["files"]["ratio_file"]["column_names"]["log2FC"]] = data[
        config["files"]["ratio_file"]["column_names"]["log2FC"]
    ] - float(options.recenter)
...
if config["files"]["ratio_file"]["column_names"]["gene"] not in data.columns:
    ...
    logging.warning(
        'Column titled "'
        + config["files"]["ratio_file"]["column_names"]["gene"]
        + '" not found in the ratio file. Using "-" in place of the annotation. '
...
if config["files"]["ratio_file"]["column_names"]["weight"] not in data.columns:
    ...
    logging.warning(
        'Column titled "'
        + config["files"]["ratio_file"]["column_names"]["weight"]
        + '" not found in the ratio file. Using "0.1" in place of the weight.'
    )
...
data[config["files"]["ratio_file"]["column_names"]["log2FC"]] = np.where(
    np.logical_and(
        data[config["files"]["ratio_file"]["column_names"]["log2FC"]]
        < config["files"]["ratio_file"]["off_target_low_conf_log2"],
        data.label == "Antitarget",
    ),
    np.nan,
    data[config["files"]["ratio_file"]["column_names"]["log2FC"]],
)

if data.label[0] == "Antitarget" and np.isnan(
    data[config["files"]["ratio_file"]["column_names"]["log2FC"]][0]
):
...
data_y_range_uniq = sorted(
    set(data[config["files"]["ratio_file"]["column_names"]["log2FC"]])
)

chr_cumsum = pd.read_csv(
    options.genome_file, sep=config["files"]["genome_file"]["field_separator"]
)
logging.info("Successfully read the genome file.")

# make sure chromosome names are string data type
chr_cumsum[config["files"]["genome_file"]["column_names"]["chromosome"]] = chr_cumsum[
    config["files"]["genome_file"]["column_names"]["chromosome"]
].astype(str)
...
data = pd.merge(
    ...
    right_on=config["files"]["genome_file"]["column_names"]["chromosome"],
    how="left",
)
data["genome_cumsum"] = (
    data[config["files"]["ratio_file"]["column_names"]["start"]]
    + data[config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]]
)
...
if options.seg_blacklist:
    ...
    seg_blacklist["genome_cumsum_start"] = (
        seg_blacklist["start"]
        + seg_blacklist[
            config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
        ]
    )
    seg_blacklist["genome_cumsum_end"] = (
        seg_blacklist["end"]
        + seg_blacklist[
            config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
        ]
...
if options.seg_file:
    seg = pd.read_csv(
        options.seg_file, sep=config["files"]["segmentation_file"]["field_separator"]
    )
    logging.info("Successfully read the genome file.")

    if options.recenter:
        seg[config["files"]["segmentation_file"]["column_names"]["log2FC"]] = seg[
            config["files"]["segmentation_file"]["column_names"]["log2FC"]
        ] - float(options.recenter)
    ...
    if config["files"]["segmentation_file"]["column_names"]["gene"] not in seg.columns:
        ...
        logging.warning(
            'Column titled "'
            + config["files"]["segmentation_file"]["column_names"]["gene"]
            + '" not found in the segmentation file. Using "-" in place of the annotation.'
        )

    # make sure chromosome names are string data type
    seg[config["files"]["segmentation_file"]["column_names"]["chromosome"]] = seg[
        config["files"]["segmentation_file"]["column_names"]["chromosome"]
    ].astype(str)
    seg["prob_start"] = data.merge(
        seg,
        how="right",
        left_on=[
            config["files"]["ratio_file"]["column_names"]["chromosome"],
    ...
    seg = pd.merge(
        seg,
        chr_cumsum,
        left_on=config["files"]["segmentation_file"]["column_names"]["chromosome"],
        right_on=config["files"]["genome_file"]["column_names"]["chromosome"],
        how="left",
    )
    seg["genome_cumsum_start"] = (
        seg[config["files"]["segmentation_file"]["column_names"]["start"]]
        + seg[config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]]
    )
    seg["genome_cumsum_end"] = (
        seg[config["files"]["segmentation_file"]["column_names"]["end"]]
        + seg[config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]]
    )
...
if options.gene_file:
    gen = pd.read_csv(
        options.gene_file, sep=config["files"]["gene_file"]["field_separator"]
    )
    logging.info("Successfully read the gene CNV file.")

    if options.recenter:
        gen[config["files"]["gene_file"]["column_names"]["log2FC"]] = gen[
            config["files"]["gene_file"]["column_names"]["log2FC"]
        ] - float(options.recenter)

    gen[config["files"]["gene_file"]["column_names"]["chromosome"]] = gen[
        config["files"]["gene_file"]["column_names"]["chromosome"]
    ].astype(str)
    gen["prob_start"] = data.merge(
        gen,
        how="right",
        left_on=[
            config["files"]["ratio_file"]["column_names"]["chromosome"],
    ...
    gen = pd.merge(
        gen,
        chr_cumsum,
        left_on=config["files"]["gene_file"]["column_names"]["chromosome"],
        right_on=config["files"]["genome_file"]["column_names"]["chromosome"],
        how="left",
    )
    gen["genome_cumsum_start"] = (
        gen[config["files"]["gene_file"]["column_names"]["start"]]
        + gen[config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]]
    )
    gen["genome_cumsum_end"] = (
        gen[config["files"]["gene_file"]["column_names"]["end"]]
        + gen[config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]]
    )
    ...
    gene_boundaries["genome_cumsum_start"] = (
        gene_boundaries.gene_start
        + gene_boundaries[
            config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
        ]
    )
    gene_boundaries["genome_cumsum_end"] = (
        gene_boundaries.gene_end
        + gene_boundaries[
            config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
        ]
    )
...
if options.annot_file:
    gene_track = pd.read_csv(
        options.annot_file, sep=config["files"]["annotation_file"]["field_separator"]
    )
    logging.info("Successfully read the annotation file.")

    gene_track[
        config["files"]["annotation_file"]["column_names"]["chromosome"]
    ] = gene_track[
        config["files"]["annotation_file"]["column_names"]["chromosome"]
    ].astype(
        str
    ...
    gene_track["genome_cumsum_start"] = (
        gene_track[config["files"]["annotation_file"]["column_names"]["exon_start"]]
        + gene_track[
            config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
        ]
    )
    gene_track["genome_cumsum_end"] = (
        gene_track[config["files"]["annotation_file"]["column_names"]["exon_end"]]
        + gene_track[
            config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
        ]
    )
...
if options.vcf_file:
    ...
    for record in vcf_reader:
        ...
        if (
            not record.ALT
            or len(record.ALT) != 1
            or (record.ALT[0] and len(record.ALT[0]) != 1)
        ):
            continue
    ...
    if not df_vaf.empty:
        ...
        if options.vcf_filt_file:
            output_VAF_filename = (
                os.path.basename(os.path.splitext(options.vcf_file)[0])
                + "_filt_SNPs.txt"
            )
            print(
                "Writing plotted variants to ",
                outdir,
                "/",
                output_VAF_filename,
                "...",
        ...
        if options.bed_blacklist:
            df_vaf = pd.merge(
                df_vaf,
                bed_blacklist,
                left_on=["chromosome", "start", "ref", "alt"],
                right_on=["chromosome", "start", "ref", "alt"],
                how="left",
            )
            df_vaf["count"].replace("", np.nan, inplace=True)
            df_vaf = df_vaf[pd.isnull(df_vaf["count"])]
        df_vaf["genome_cumsum"] = (
            df_vaf.start
            + df_vaf[
                config["files"]["genome_file"]["column_names"]["chr_cumulative_length"]
            ]
        ...
        if options.seg_file:
            df_vaf_seg = pd.merge(
                ...
                seg,
                left_on="chromosome",
                right_on=config["files"]["segmentation_file"]["column_names"][
                    "chromosome"
                ],
                how="left",
            )
            if config["files"]["segmentation_file"]["column_names"]["start"] == "start":
                df_vaf_seg["intersect"] = np.where(
                    (
                        (df_vaf_seg["start_x"] >= df_vaf_seg["start_y"])
                        & (
                            df_vaf_seg["start_x"]
                            <= df_vaf_seg[
                                config["files"]["segmentation_file"]["column_names"][
                                    "end"
                                ]
                            ]
                        )
                ...
                df_vaf_count = (
                    df_vaf_seg.groupby(["chromosome", "start_y", "end", "cluster"])
                    .count()
                    .reset_index()
                    .intersect
                )
        ...
        if options.gene_file:
            ...
            df_vaf_gene["intersect"] = np.where(
                (
                    ...
                    & (df_vaf_gene["start"] <= df_vaf_gene["gene_end"])
                ),
                1,
                0,
            )
        ...
        logging.warning(
            "No variants to plot! (VCF did not have any variants passing the set thresholds)"
        )
...
source = ColumnDataSource(
    data=dict(
        ...
        * config["files"]["ratio_file"]["weight_scaling_factor"],
    )
)
...
if options.seg_file and (
    (
        config["files"]["segmentation_file"]["column_names"]["major_cn"] in seg.columns
        and config["files"]["segmentation_file"]["column_names"]["minor_cn"]
        in seg.columns
    )
    and config["files"]["segmentation_file"]["column_names"]["cell_frac"] in seg.columns
):
    source_seg = ColumnDataSource(
        data=dict(
            ...
            major_cn=seg[
                config["files"]["segmentation_file"]["column_names"]["major_cn"]
            ],
            minor_cn=seg[
                config["files"]["segmentation_file"]["column_names"]["minor_cn"]
            ],
            cell_frac=seg[
                config["files"]["segmentation_file"]["column_names"]["cell_frac"]
            ],
...
# if only integer copy number information is present
    ...
    config["files"]["segmentation_file"]["column_names"]["major_cn"] in seg.columns
    and config["files"]["segmentation_file"]["column_names"]["minor_cn"] in seg.columns
):
    source_seg = ColumnDataSource(
        data=dict(
            chrom=seg[
...
# if only clonality information is present
    ...
    source_seg = ColumnDataSource(
        data=dict(
            ...
            genome_cumsum_end=seg.genome_cumsum_end,
            cell_frac=round(
                seg[config["files"]["segmentation_file"]["column_names"]["cell_frac"]],
                2,
            ),
            gene=seg[config["files"]["segmentation_file"]["column_names"]["gene"]],
        )
...
if (
    options.seg_blacklist
    and config["plots"]["logFC_genome_plot"]["artifact_mask"]["visibility"] == "on"
):
    source_seg_blacklist = ColumnDataSource(
        data=dict(
...
if options.annot_file:
    source_gene_track = ColumnDataSource(
        data=dict(
            chrom=gene_track[
                config["files"]["annotation_file"]["column_names"]["chromosome"]
            ],
            start=gene_track[
                config["files"]["annotation_file"]["column_names"]["exon_start"]
            ],
            end=gene_track[
                config["files"]["annotation_file"]["column_names"]["exon_end"]
            ],
            txid=gene_track[
                config["files"]["annotation_file"]["column_names"]["tx_id"]
            ],
            exonNum=gene_track[
                config["files"]["annotation_file"]["column_names"]["exon_number"]
            ],
...
if options.vcf_file and not df_vaf.empty:
    ...
    if options.seg_file:
        if config["files"]["segmentation_file"]["column_names"]["start"] == "start":
            source_vaf_seg = ColumnDataSource(
                data=dict(
                    chrom=df_vaf_seg[
                        config["files"]["segmentation_file"]["column_names"][
                            "chromosome"
    ...
    if options.gene_file:
        source_vaf_gene = ColumnDataSource(
            data=dict(
...
logFC = figure(
    plot_width=config["plots"]["logFC_ind_plot"]["width"],
    ...
    x_range=DataRange1d(
        bounds=(
            min(data["ind"]) - 0.05 * max(data["ind"]),
            max(data["ind"]) + 0.05 * max(data["ind"]),
        )
    ),
    y_range=Range1d(
        (data_y_range_uniq[3:] or [0])[0] - 0.5,
        (data_y_range_uniq[-4:] or [0])[0] + 0.5,
        bounds=(
            min(data[config["files"]["ratio_file"]["column_names"]["log2FC"]]) - 1,
...
if (
    options.gene_file
    and config["plots"]["logFC_ind_plot"]["gene_markers"]["visibility"] == "on"
):
    logFC.quad(
        top="logFC",
        bottom=0,
...
logFC.circle(
    ...
    fill_color=factor_cmap(
        "label",
        palette=[
            config["plots"]["logFC_ind_plot"]["point_on_target_color"],
...
if (
    options.seg_file
    and config["plots"]["logFC_ind_plot"]["segment_markers"]["visibility"] == "on"
):
    logFC.segment(
        x0="prob_start",
        y0="logFC",
        x1="prob_end",
        y1="logFC",
        color=config["plots"]["logFC_ind_plot"]["segment_markers"][
            "segment_line_color"
        ],
        line_width=config["plots"]["logFC_ind_plot"]["segment_markers"][
            "segment_line_width"
        ],
        level="annotation",
        alpha=config["plots"]["logFC_ind_plot"]["segment_markers"][
            "segment_line_alpha"
        ],
        source=source_seg,
    )  # segmentation line
...
logFC.xaxis.visible = (
    config["plots"]["logFC_ind_plot"]["x_axis_label_visibility"] == "on"
)
logFC.yaxis.visible = (
    config["plots"]["logFC_ind_plot"]["y_axis_label_visibility"] == "on"
)
...
logFC_genome = figure(
    plot_width=config["plots"]["logFC_genome_plot"]["width"],
    ...
    x_range=DataRange1d(
        bounds=(
            min(data.genome_cumsum) - 0.05 * max(data.genome_cumsum),
    ...
    title=config["plots"]["logFC_genome_plot"]["title"]
    + " ("
    + title_name
    + ")"
    + "\t\tGender: "
    ...
    + "\t\tPloidy: "
    + str(options.ploidy)
    if config["plots"]["logFC_genome_plot"]["title"]
    else "",
)

if (
    options.gene_file
    and config["plots"]["logFC_genome_plot"]["gene_markers"]["visibility"] == "on"
):
    logFC_genome.quad(
        top="logFC",
        bottom=0,
...
logFC_genome.circle(
    ...
    fill_color=factor_cmap(
        "label",
        palette=[
            config["plots"]["logFC_genome_plot"]["point_on_target_color"],
...
if (
    options.seg_file
    and config["plots"]["logFC_genome_plot"]["segment_markers"]["visibility"] == "on"
):
    logFC_genome.segment(
        x0="genome_cumsum_start",
        y0="logFC",
        x1="genome_cumsum_end",
        y1="logFC",
        color=config["plots"]["logFC_genome_plot"]["segment_markers"][
            "segment_line_color"
        ],
        line_width=config["plots"]["logFC_genome_plot"]["segment_markers"][
            "segment_line_width"
        ],
        level="annotation",
        alpha=config["plots"]["logFC_genome_plot"]["segment_markers"][
            "segment_line_alpha"
        ],
        source=source_seg,
    )  # segmentation line
...
if (
    ...
    # print("Entered!")
    logFC_genome.quad(
        top=max(data[config["files"]["ratio_file"]["column_names"]["log2FC"]]),
...
logFC_genome.xaxis.visible = (
    config["plots"]["logFC_genome_plot"]["x_axis_label_visibility"] == "on"
)
logFC_genome.yaxis.visible = (
    config["plots"]["logFC_genome_plot"]["y_axis_label_visibility"] == "on"
)
...
if options.seg_file:
    int_cn_visibitlity = (
        config["plots"]["int_cn_plot"]["visibility"] == "on"
        and config["files"]["segmentation_file"]["column_names"]["major_cn"]
        in seg.columns
        and config["files"]["segmentation_file"]["column_names"]["minor_cn"]
        in seg.columns
    )
    ...
    int_cn_flag = (
        options.seg_file
        and config["plots"]["int_cn_plot"]["visibility"] == "on"
        and (
            (
                config["files"]["segmentation_file"]["column_names"]["major_cn"]
                in seg.columns
            ...
            or config["files"]["segmentation_file"]["column_names"]["cell_frac"]
            in seg.columns
        )
    )
...
if options.seg_file and int_cn_flag:
    int_cn_genome = figure(
        plot_width=config["plots"]["int_cn_plot"]["width"],
    ...
    if (
        ...
        int_cn_genome.segment(
            x0="genome_cumsum_start",
            y0="minor_cn",
            x1="genome_cumsum_end",
            y1="minor_cn",
            color=config["plots"]["int_cn_plot"]["segment_markers"][
                "minor_segment_line_color"
            ],
            line_width=config["plots"]["int_cn_plot"]["segment_markers"][
                "minor_segment_line_width"
            ],
            level="annotation",
            alpha=config["plots"]["int_cn_plot"]["segment_markers"][
                "minor_segment_line_alpha"
            ],
            source=source_seg,
        )  # segmentation line
        ...
        int_cn_genome.segment(
            ...
            y1="major_cn",
            color=config["plots"]["int_cn_plot"]["segment_markers"][
                "major_segment_line_color"
            ],
            line_width=config["plots"]["int_cn_plot"]["segment_markers"][
                "major_segment_line_width"
            ],
            level="annotation",
            alpha=config["plots"]["int_cn_plot"]["segment_markers"][
                "major_segment_line_alpha"
            ],
            source=source_seg,
        )  # segmentation line
    ...
    if config["files"]["segmentation_file"]["column_names"]["cell_frac"] in seg.columns:
        int_cn_genome.quad(
            top=max(
                seg[config["files"]["segmentation_file"]["column_names"]["major_cn"]]
            ),
            bottom=min(
                seg[config["files"]["segmentation_file"]["column_names"]["minor_cn"]]
            ),
            left="genome_cumsum_start",
            right="genome_cumsum_end",
    ...
    int_cn_genome.xaxis.visible = (
        config["plots"]["int_cn_plot"]["x_axis_label_visibility"] == "on"
    )
    int_cn_genome.yaxis.visible = (
        config["plots"]["int_cn_plot"]["y_axis_label_visibility"] == "on"
    )
...
#############
if options.annot_file:
    logFC_genome_gene_track = figure(
        plot_width=config["plots"]["annotation_plot"]["width"],
    ...
    logFC_genome_gene_track.segment(
        x0="genome_cumsum_start",
        y0=0,
        x1="genome_cumsum_end",
        y1=0,
...
if options.vcf_file and not df_vaf.empty:
    VAF_genome = figure(
        plot_width=config["plots"]["vaf_plot"]["width"],
    ...
    if (
        options.seg_file
        and config["plots"]["vaf_plot"]["segment_markers"]["visibility"] == "on"
    ):
        VAF_genome.segment(
            x0="genome_cumsum_start",
            y0="AF",
            ...
            line_width=config["plots"]["vaf_plot"]["segment_markers"][
                "segment_line_width"
            ],
            level="annotation",
            alpha=config["plots"]["vaf_plot"]["segment_markers"]["segment_line_alpha"],
            source=source_vaf_seg,
        )  # segmentation line

    if (
        options.gene_file
        and config["plots"]["vaf_plot"]["gene_markers"]["visibility"] == "on"
    ):
        VAF_genome.segment(
            x0="genome_cumsum_start",
            y0="AF",
    ...
    VAF_genome.xaxis.visible = (
        config["plots"]["vaf_plot"]["x_axis_label_visibility"] == "on"
    )
    VAF_genome.yaxis.visible = (
        config["plots"]["vaf_plot"]["y_axis_label_visibility"] == "on"
    )
...
chr_boundary = (
    pd.concat(
        [data[config["files"]["ratio_file"]["column_names"]["chromosome"]], data.ind],
        axis=1,
    )
    .groupby(
        [config["files"]["ratio_file"]["column_names"]["chromosome"]], as_index=False
    )
    .max()
)
# print(chr_boundary)
...
if options.gene_file:
    ...
    selection_options.extend(
        np.unique(gen[config["files"]["gene_file"]["column_names"]["gene"]].tolist())
    )

    select = MultiSelect(
        title="Gene filter", options=selection_options, max_width=300, size=15
    )

    filteredSource = ColumnDataSource(
        data=dict(chrom=[], start=[], end=[], gene=[], logFC=[])
    )
    ...
    callback_filter = CustomJS(
        args=dict(
            ...
            amp_threshold=config["files"]["gene_file"]["amp_threshold"],
        ),
        code="""
...
if options.vcf_file and options.annot_file and options.gene_file and not df_vaf.empty:
    logging.info(
        "options.vcf_file and options.annot_file and options.gene_file and not df_vaf.empty"
    )
    if int_cn_flag:
        plots = gridplot(
            [
                [logFC_genome],
                [logFC_genome_gene_track],
                [VAF_genome],
                [int_cn_genome],
                [logFC],
            ],
        ...
        plots = gridplot(
            [[logFC_genome], [logFC_genome_gene_track], [VAF_genome], [logFC]],
            toolbar_location="left",
            merge_tools=True,
        )
    fig_datatables = layout(
        row(
            column(table_gene_div, data_table_gene),
            column(table_data_div, data_table),
            column(select),
        )
    )
    ...
    fig_datatables = layout(row(column(table_bin_div, data_table_cnr)))
    final_fig = layout(children=[[plots], [fig_datatables]])
    ...
    if int_cn_flag:
        plots = gridplot(
            [[logFC_genome], [VAF_genome], [int_cn_genome], [logFC]],
            toolbar_location="left",
            merge_tools=True,
        )
    else:
        plots = gridplot(
            [[logFC_genome], [VAF_genome], [logFC]],
            toolbar_location="left",
            merge_tools=True,
        )
    fig_datatables = layout(
    ...
    if int_cn_flag:
        plots = gridplot(
            [[logFC_genome], [logFC_genome_gene_track], [int_cn_genome], [logFC]],
            toolbar_location="left",
            merge_tools=True,
        )
    else:
        plots = gridplot(
            [[logFC_genome], [logFC_genome_gene_track], [logFC]],
            toolbar_location="left",
            merge_tools=True,
        )
    fig_datatables = layout(
    ...
    if int_cn_flag:
        plots = gridplot(
            [[logFC_genome], [int_cn_genome], [logFC]],
            toolbar_location="left",
            merge_tools=True,
        )
    else:
        plots = gridplot(
            [[logFC_genome], [logFC]], toolbar_location="left", merge_tools=True
        )
    fig_datatables = layout(
        row(
            column(table_gene_div, data_table_gene),
